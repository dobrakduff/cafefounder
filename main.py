...
@app.route("/all")
def get_all_coffee():
    result = db.session.query(Cafe).order_by(Cafe.name).all()
    all_cafes = [cafe_to_dict(cafe) for cafe in result]
...
def search_caffe():
    ...
    all_cafes = result.scalars().all()
    if all_cafes:
        cafes = [cafe_to_dict(cafe) for cafe in all_cafes]
